...
"""
Copied from snapshot_operations
"""
...
"""
Wrapper function added to clone VM from a Snapshot
"""
def clone_vm_from_snapshot(si, datacenter_name, cluster_name, host_name, template_name, vm_name, vm_folder):

    content = si.RetrieveContent()
    
    datacenter = get_obj(content, [vim.Datacenter], datacenter_name)
    if not datacenter:
        raise Exception("Couldn't find the Datacenter with the provided name {}'".format(datacenter_name))

    cluster = get_obj(content, [vim.ClusterComputeResource], cluster_name, datacenter.hostFolder)
    if not cluster:
        raise Exception("Couldn't find the Cluster with the provided name '{}'".format(cluster_name))
    ...
    #template = get_obj(content, [vim.VirtualMachine], template_name, vm_folder)

    template = get_obj(content, [vim.VirtualMachine], template_name)
    if not template:
        raise Exception("Couldn't find the template with the provided name '{}'".format(template_name))
    ...
    _clone_vm(si, template, vm_name, dest_folder, location)


def _clone_vm(si, template, vm_name, vm_folder, location):
    
    #Get the name of current snapshot
    current_snap_obj = get_current_snap_obj(template.snapshot.rootSnapshotList, template.snapshot.currentSnapshot)
    print("Will use current snapshot named '{}'".format(current_snap_obj[0].name))
    
    clone_spec = vim.vm.CloneSpec(
        powerOn=True, template=False, location=location, snapshot=template.snapshot.currentSnapshot)
