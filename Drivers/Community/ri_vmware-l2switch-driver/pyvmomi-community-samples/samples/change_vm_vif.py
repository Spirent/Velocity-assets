...
""" 
Added this new function to change the network adapter of the VM
""" 
def change_vm_network(service_instance, uuid, mac_address, network_name, is_DVS):
    print("Changing the network of MAC Address " + mac_address + " to '" + network_name + "'.")
        ...
        if isinstance(device, vim.vm.device.VirtualEthernetCard):
            if device.macAddress == mac_address:
                ...
                if network_name != "":
                    ...
                    #If network_name is empty, we assume we want to remove/detach the newtwork from the NIC
...
def get_VSS_network(content, network_name):
    network = get_obj(content, [vim.Network], network_name)
    return network

def get_VDS_network(content, network_name):
    network = get_obj(content, [vim.dvs.DistributedVirtualPortgroup], network_name)    
    return network
