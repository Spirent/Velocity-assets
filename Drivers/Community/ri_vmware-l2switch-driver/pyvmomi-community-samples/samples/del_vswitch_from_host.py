...
"""
Added to return a host/cluster that matches the given name.
"""
def GetHost(hosts, clusterName):
    for host in hosts:
        if host.name == clusterName:
            return host
...
def DelHostSwitch(host, vswitchName):
    print("Deleting switch '" + vswitchName + "' from host '" + host.name + "'")
